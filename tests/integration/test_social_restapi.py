...
class MessagesApiTest(BaseApiTest):
  ...
  def test_create_message(self):
    ...
    # Test message exists
    response = self.client.get("/api/messages/%d" % m['id'])
    self.assert_200(response)
    m1 = response.json
    for k in m1:
    ...
    message = messages[0]
    self.assertEquals(message['creator_id'], self.master_user.id)

    # Test stream
    response = self.client.get("/api/users/%d/messages" % self.master_user.id)
    self.assert_200(response)
    messages = response.json
    self.assertEquals(len(messages), 1)
  ...
  def test_update_message(self):
    ...
    # Test update
    response = self.client.put("/api/messages/%d" % m['id'], data={'content': 'Changed'})
    self.assert_200(response)
    m2 = response.json
    content = m2['content']
  ...
  def test_delete_message(self):
    ...
    # Test delete
    response = self.client.delete("/api/messages/%d" % m['id'])
...
class UserApiTest(BaseApiTest):
  ...
  def test_create_user(self):
    ...
    # Test user exists
    response = self.client.get("/api/users/%d" % u['id'])
    self.assert_200(response)
    u1 = response.json
    for k in u1:
  ...
  def test_update_user(self):
    ...
    # Test update
    response = self.client.put("/api/users/%d" % u['id'], data={'first_name': 'Paul'})
    self.assert_200(response)
    u2 = response.json
  ...
  def test_delete_user(self):
    ...
    # Test delete
    response = self.client.delete("/api/users/%d" % u['id'])
    self.assert_204(response)
  ...
  def test_following(self):
    ...
    response = self.client.get("/api/users/%d/followers" % u['id'])
    self.assert_200(response)
    self.assertEquals(len(response.json), 0)

    response = self.client.get("/api/users/%d/followees" % u['id'])
    self.assert_200(response)
    self.assertEquals(len(response.json), 0)

    # Follow new user
    response = self.client.post("/api/users/%d/followers" % u['id'])
    self.assert_204(response)
    ...
    response = self.client.get("/api/users/%d/followees" % self.master_user.id)
    self.assert_200(response)
    self.assertEquals(len(response.json), 1)

    # Unfollow
    response = self.client.delete("/api/users/%d/followers/%d" % (u['id'], self.master_user.id))
    self.assert_204(response)
...
class GroupApiTest(BaseApiTest):
  ...
  def test_create_group(self):
    ...
    # Test user exists
    response = self.client.get("/api/groups/%d" % group['id'])
    self.assert_200(response)
