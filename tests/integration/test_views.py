...
class TestViews(IntegrationTestCase):
  ...
  def test_users(self):
    ...
    response = self.client.get("/users/%d" % id)
    self.assert_200(response)

    response = self.client.get("/users/%d/mugshot" % id)
    self.assert_200(response)
    eq_("image/jpeg", response.headers["content-type"])

    response = self.client.get("/users/%d/mugshot?s=55" % id)
    self.assert_200(response)
    eq_("image/jpeg", response.headers["content-type"])

    response = self.client.get("/users/%d/mugshot?s=48" % id)
    self.assert_200(response)
    eq_("image/jpeg", response.headers["content-type"])
